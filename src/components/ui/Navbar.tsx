...
export default function Navbar() {
  ...
  return (
    <nav className="fixed top-0 w-full z-50">
      {/* 배경 */}
      <div className="absolute inset-0 bg-[#0a0a0a]/80 backdrop-blur-2xl border-b border-white/[0.07]" />

      <div className="relative max-w-6xl mx-auto px-5 h-[60px] flex items-center justify-between gap-6">

        {/* 로고 */}
        <Link href="/" className="flex items-center gap-2.5 shrink-0">
          <div className="w-7 h-7 rounded-lg bg-gradient-to-br from-amber-400 to-orange-500 flex items-center justify-center text-black text-xs font-black shadow-lg shadow-amber-500/25">
            다
          </div>
          <span className="text-white font-bold text-[17px] tracking-tight">다잇다</span>
        </Link>

        {/* 데스크탑 중앙 네비 */}
        <div className="hidden md:flex items-center gap-0.5 bg-white/[0.04] rounded-2xl px-1.5 py-1.5 border border-white/[0.07]">
          {NAV_LINKS.map(({ href, label, sub, icon, accent, activeBg, dot }) => {
            ...
            return (
              ...
              >
                {/* 아이콘: 항상 표시 */}
                <span className={active ? accent : 'text-white/30'}>
                  {icon}
                </span>
                {/* lg+: 라벨 표시 */}
                ...
                <span className="hidden xl:flex flex-col leading-none gap-0.5">
                  ...
                  <span className={`text-[10px] font-normal ${active ? 'opacity-60' : 'text-white/25'}`}>{sub}</span>
                </span>
        ...
        <div className="flex items-center gap-1.5 shrink-0">
          ...
          <Link
            href="/search"
            className="w-9 h-9 flex items-center justify-center rounded-xl text-white/40 hover:text-white hover:bg-white/6 transition border border-transparent hover:border-white/8"
            title="검색"
          >
            <SearchIcon />
          </Link>
          ...
          {isLoading ? (
            ...
            <div className="relative">
              <button
                ...
                className="flex items-center gap-2 px-3 py-1.5 rounded-xl hover:bg-white/6 transition border border-transparent hover:border-white/8"
              >
                {user.user_metadata?.avatar_url ? (
                  ...
                  <div className="w-6 h-6 rounded-full bg-gradient-to-br from-amber-400/40 to-orange-500/40 border border-white/15 flex items-center justify-center text-white text-[10px] font-bold">
                    {(user.user_metadata?.full_name || user.email || '?')[0].toUpperCase()}
                  </div>
                )}
                <span className="text-white/60 text-[13px] hidden sm:block max-w-[80px] truncate">
              ...
              {/* 유저 드롭다운 */}
              {userMenuOpen && (
                <div className="absolute right-0 top-full mt-2 w-48 rounded-2xl bg-[#141414] border border-white/10 shadow-xl shadow-black/50 overflow-hidden py-1.5">
                  <Link
                    href="/dashboard"
                  ...
                  <button
                    ...
                    className="w-full flex items-center gap-2.5 px-4 py-2.5 text-white/40 text-sm hover:text-rose-400 hover:bg-rose-500/5 transition"
                  >
                    <svg className="w-[18px] h-[18px]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div className="flex items-center gap-1.5">
              <Link
                href="/auth/login"
                className="hidden sm:block text-white/50 text-[13px] hover:text-white transition px-3 py-1.5 rounded-xl hover:bg-white/5"
              >
                로그인
              </Link>
              <Link
                href="/auth/signup"
                className="text-[13px] font-semibold px-4 py-1.5 rounded-xl bg-white text-black hover:bg-white/90 transition shadow-lg shadow-white/5"
              >
                회원가입
              </Link>
            </div>
          ...
          {/* 모바일 햄버거 */}
          <button
            className="md:hidden w-9 h-9 flex flex-col items-center justify-center gap-[5px] rounded-xl hover:bg-white/5 transition ml-0.5"
            onClick={() => setMenuOpen((v) => !v)}
          >
            <span className={`w-[18px] h-0.5 bg-white/50 rounded-full transition-all duration-200 ${menuOpen ? 'rotate-45 translate-y-[7px]' : ''}`} />
            <span className={`w-[18px] h-0.5 bg-white/50 rounded-full transition-all duration-200 ${menuOpen ? 'opacity-0' : ''}`} />
            <span className={`w-[18px] h-0.5 bg-white/50 rounded-full transition-all duration-200 ${menuOpen ? '-rotate-45 -translate-y-[7px]' : ''}`} />
          </button>
        </div>
      </div>
      ...
      {/* 모바일 메뉴 */}
      {menuOpen && (
        <div className="md:hidden relative bg-[#0d0d0d]/95 backdrop-blur-xl border-b border-white/[0.07] px-5 py-4 flex flex-col gap-1">
          {NAV_LINKS.map(({ href, label, accent }) => (
            <Link
              ...
              className={`px-4 py-3 rounded-xl text-[13px] font-medium transition ${
                isActive(href) ? `${accent} bg-white/6` : 'text-white/55 hover:text-white hover:bg-white/4'
              }`}
            >
              {label}
            </Link>
