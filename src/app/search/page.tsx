    ...
    <main className="min-h-screen bg-[#0a0a0a] pt-20">
      <div className="max-w-2xl mx-auto px-6 py-12">
        ...
        <div className="flex gap-2 mb-8">
          <div className="flex-1 flex items-center gap-3 px-4 rounded-xl bg-white/5 border border-white/10 focus-within:border-white/25 transition">
            ...
            <input
              ...
              className="flex-1 py-3.5 bg-transparent text-white placeholder-white/25 focus:outline-none text-sm"
            />
            {query && (
              <button onClick={() => { setQuery(''); setResults([]); setSearched(false); }} className="text-white/20 hover:text-white/50 transition shrink-0">
          ...
          <button
            ...
            className="px-5 py-3.5 rounded-xl bg-white text-black text-sm font-bold hover:bg-white/90 transition disabled:opacity-40"
          >
            {loading ? (
        ...
        {searched && (
          <>
            {/* 탭 필터 */}
            <div className="flex items-center gap-1 mb-5 bg-white/[0.03] rounded-xl p-1 border border-white/[0.06]">
              {TABS.map((tab) => (
                <button
                  key={tab}
                ...
                >
                  ...
                  {counts[tab] > 0 && (
                    <span className={`text-[11px] px-1.5 py-0.5 rounded-full ${
                      activeTab === tab ? 'bg-white/15 text-white/80' : 'bg-white/6 text-white/30'
                    }`}>
                      {counts[tab]}
                    </span>
                  )}
            ...
            {filtered.length === 0 ? (
              <div className="text-center py-20">
                <p className="text-white/30 text-sm">"{query}"에 대한 {activeTab === '전체' ? '' : `${activeTab} `}결과가 없습니다</p>
              </div>
            ) : (
                ...
                {filtered.map((r) => {
                  ...
                  return (
                    <Link
                      key={`${r.type}-${r.id}`}
                      href={r.href}
                      className="flex items-start gap-3 p-4 rounded-xl border border-white/6 bg-white/[0.02] hover:bg-white/5 hover:border-white/12 transition group"
                    >
                      <span className={`text-[11px] font-semibold px-2 py-0.5 rounded-full shrink-0 mt-0.5 ${meta.color} ${meta.bg}`}>
                        {meta.label}
                      </span>
                      <div className="flex-1 min-w-0">
                        <p className="text-white text-sm font-medium group-hover:text-white/90 leading-snug">
                        ...
                        {r.badge && (
                          <span className="inline-block mt-1.5 text-[10px] text-white/25 bg-white/5 px-2 py-0.5 rounded-full">
                            {r.badge}
                          </span>
                        )}
                      </div>
                      <svg className="w-4 h-4 text-white/15 group-hover:text-white/40 transition shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        {!searched && (
          <div className="flex flex-col items-center justify-center py-24 gap-3">
            <svg className="w-10 h-10 text-white/8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
