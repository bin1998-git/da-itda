    ...
    <main className="min-h-screen bg-[#0a0a0a] pt-20">
      <div className="max-w-5xl mx-auto px-6 py-12">
        ...
        <div className="mb-10">
          ...
          <h1 className="text-3xl font-bold text-white">인기 TOP 10</h1>
          <p className="text-white/35 text-sm mt-2">조회수 기준 가장 많이 본 콘텐츠</p>
        </div>
          ...
          {tabs.map(({ key, label, color, items }) => (
            <div key={key}>
              ...
              <ol className="flex flex-col gap-2">
                {items.length === 0 ? (
                  <li className="text-white/20 text-sm py-6 text-center">데이터가 없습니다</li>
                  ...
                  <li key={item.id as string}>
                    <Link
                      ...
                      className="flex items-center gap-3 p-3 rounded-xl hover:bg-white/4 transition group"
                    >
                      <span className={`text-lg font-black w-7 text-center shrink-0 ${idx < 3 ? color : 'text-white/20'}`}>
                        {idx + 1}
                      </span>
                      <span className="text-white/70 text-sm truncate group-hover:text-white transition">
